		...
		case T_InClauseInfo:
			{
				...
				if (walker(ininfo->sub_targetlist, context))
					return true;
			}
			break;
		case T_ColumnRef:
		...
		case T_WindowSpec:
			{
				...
				if (walker(wspec->partition, context))
					return true;
				if (walker(wspec->order, context))
					return true;
				if (walker(wspec->frame, context))
					return true;
				return false;
			}
			break;
		case T_WindowFrame:
			{
				...
				if (walker(frame->trail, context))
					return true;
				if (walker(frame->lead, context))
					return true;
			}
			break;
		case T_WindowKey:
